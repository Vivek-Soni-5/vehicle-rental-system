...
$sql_query = "SELECT `vehicle`.`model_name`,`book`.`customer_name`,`customer_signup`.`email`,`cust_ph_no`.`phone` FROM `vehicle`,`book`,`customer_signup`,`cust_ph_no` WHERE `vehicle`.`owner_id`=$owner_id AND `vehicle`.`booked`='yes' AND `vehicle`.`v_id`=`book`.`v_id` AND `book`.`c_id`=`customer_signup`.`c_id` AND `cust_ph_no`.`cust_id`=`customer_signup`.`c_id`;";
    ...
    ?>
                <div class="p-6">
                    ...
                    <p class="text-gray-700 text-base mb-4">
                        ...
                        Customer Email : <?php echo ($row['email'])?><br>
                    </p>
                </div>
            </div>
        </div>
